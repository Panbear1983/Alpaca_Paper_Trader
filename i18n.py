...
_TABLE: dict[str, tuple[str, str]] = {
    ...
    "rpt.score": ("🎯 Since {since}: wallet {w} · QQQ {b} · gap {gap} pts",
                  "🎯 自 {since}：錢包 {w} · QQQ {b} · 差距 {gap} 點"),
    "rpt.score_src": ("   realized by who opened the trade: {parts}",
                      "   已實現損益（依開倉者）：{parts}"),
...
_FIELD_ZH: dict[str, tuple[str, str]] = {
    ...
    "anchor.cooling_off_pct": (
        "冷卻期觸發（低於 20 日高點 %）",
        "權益低於 20 日高點達此幅度時暫停新買入（賣出與停損照常）。創新高後才會重新啟用。0 = 關閉。"),
    "anchor.cooling_off_days": (
        "冷卻期長度（交易日）",
        "觸發後暫停新買入的交易日數。"),
    "anchor.max_open_names": (
        "同時持有上限（檔數）",
        "已持有達此檔數時，拒絕買入新代號。白名單是選單，不是投資組合。0 = 關閉。"),
    "anchor.position_pct": (
        "機器人每筆買入規模（權益比例）",
        "錨定引擎（機器人）每筆進場佔權益的比例；試行期為 0.05。與現金底線無關。"),
    "anchor.min_cash_pct": (
        "現金底線：至少保留這麼多現金（比例）",
        "買入後現金若低於權益的此比例則拒絕。0 = 關閉。"),
